...
def create_widgets_in_first_frame():
    ...
    first_window_label = tkinter.ttk.Label(first_frame, text='What do you want to do?')
    first_window_label.grid(column=0, row=0, pady=10, padx=10, sticky=(tkinter.N))
    
    # Create the button for the frame
    first_window_quit_button = tkinter.Button(first_frame, text = "Cancel", command = quit_program)
    ...
    lb = tkinter.Listbox(first_frame)
    for i in acts:
...
def create_widgets_in_third_frame():
    ...
    third_window_quit_button = tkinter.Button(third_frame, text = "Execute", command = quit_program)
